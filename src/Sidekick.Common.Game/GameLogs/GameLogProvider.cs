...
{
    public class GameLogProvider : IGameLogProvider
    {
        private readonly IProcessProvider processProvider;
        private readonly ILogger<GameLogProvider> logger;

        public GameLogProvider(
            IProcessProvider processProvider,
            ILogger<GameLogProvider> logger)
        {
            ...
            try
            {
                ...
                while (stream.Position > 0)
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        return match.Value[(match.Value.LastIndexOf(" ") + 1)..];
                    }
